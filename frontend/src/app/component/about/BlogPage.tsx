...
export default function BlogPage() {
  ...
  {/* Navbar - remains unchanged */}
      <nav
        ...
        style={{ top: isSticky ? "10px" : "1.5rem", transition: "top 0.3s ease-in-out" }}
      >
        <Link href="/">
          <img
        ...
        <div className="flex items-center">
          {showBurgerMenu && (
            <button onClick={toggleMenu} className="text-white focus:outline-none" aria-expanded={isOpen} aria-label={isOpen ? "Close menu" : "Open menu"}>
              {isOpen ? <X size={24} /> : <Menu size={24} />}
            </button>
          )}
        </div>

        <ul className={`flex text-sm transition-all duration-300 ${showBurgerMenu
        ...
        >
          ...
          {!showBurgerMenu && (
            <li>
              <Link href="/login">
                <ButtonAnimatedGradient text="Sign-In" gradientFrom="#0f1747" gradientTo="#0f1747" borderColor="#b2b8f6" className="w-[135px] h-[40px] rounded-full font-normal block" />
              </Link>
            </li>
          )}
        </ul>
      ...
      {/* Cover Image */}
      <motion.div style={{ backgroundImage }} className="relative h-[400px] md:h-[400px] lg:h-[470px] w-full overflow-hidden">
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-center z-10">
          <h1 className="text-3xl md:text-6xl lg:text-7xl font-extrabold text-white mb-4">Blog</h1>
          <p className="text-sm md:text-xl lg:text-2xl text-gray-300">Insights and Innovations in AI and Technology</p>
        </div>
      </motion.div>
      ...
      <div className="container mx-auto py-12 px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {blogs.map(blog => (
            <motion.div key={blog.id} whileHover={{ scale: 1.03, y: -10 }} whileTap={{ scale: 0.98 }}
            ...
            >
              <div className="relative overflow-hidden rounded-2xl shadow-xl bg-transparent transform transition-shadow duration-300">
                <div className="relative">
                  <img src={blog.image} alt={blog.title} className="w-full h-[150px] sm:h-[180px] md:h-56 object-contain sm:object-cover" />
